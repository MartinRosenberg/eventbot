...
export async function handleMessage(me: ClientUser, m: Message<boolean>) {
	if (!m.content) {
		return // if it wasn't for us, we can't see the content
	}
	const channelName = (
		m.guild?.channels.cache.find((c) => c.id === m.channelId)?.name || ""
	).toLowerCase()
	if (channelName !== listenChannel.toLowerCase()) {
...
export async function handleButtonClick(intn: ButtonInteraction<CacheType>) {
	...
	;(async () => {
		...
		const resp = await intn.reply({
			content: `\`${intn.customId}\` not implemented yet.`,
			ephemeral: true,
		})
...
async function createEventFromDraft(intn: ButtonInteraction<CacheType>) {
	...
	const confirmContent = {
		content: `*Created new server event:*\n${ppEvent(v)}`,
	}
...
export async function handleModalSubmit(
	intn: ModalSubmitInteraction<CacheType>
) {
	const log = glog.child({ from: intn.user.tag })
	if (!intn.message) {
	...
	const loading = intn.reply({
		content: "Updating your event data. Please wait...",
		ephemeral: true,
	})
	log.info({ updateInfo }, "parsing event update")
	const resp = await parseEditEvent({
		...now(),
...
async function discardDraft(intn: ButtonInteraction<CacheType>) {
	...
	intn.reply({
		content:
			`Your event draft ${forName} was successfully discarded.`.replaceAll(
				/\s+/g,
				" "
			),
		ephemeral: true,
...
function buildActionButtons() {
	return new ActionRowBuilder<ButtonBuilder>().addComponents(
		new ButtonBuilder()
			...
			.setCustomId("createEventBtn"),
		new ButtonBuilder()
			.setLabel("Edit Details")
			.setStyle(ButtonStyle.Primary)
			.setCustomId("editDetailsBtn"),
		new ButtonBuilder()
			.setLabel("Discard Draft")
			.setStyle(ButtonStyle.Danger)
			.setCustomId("discardDraftBtn")
	)
}
